...
var arrayLanguage = [], // массив в который собирается названия файлов с переводами, из него создается массив используемых языков
    ...
    saveMassiveCss = [], // массив для сss
    saveMassive = [], // массив для переименования изображений
    rndNumArray = [], // массив с рандомными числами для блока FAQ
...
/**
 * Генерация рандомных чисел и запись в массив
 * Массив используется в блоке FAQ
 */
 async function getRndNumArray() {
    let articlesNumber = 30;

    while (rndNumArray.length < articlesNumber) {
        let rndNum = Math.floor(Math.random() * articlesNumber) + 1,
            found = false;

        for (let value of rndNumArray) {
            if (value === rndNum) {
                found = true;
                break;
            }
        ...
        if (!found) rndNumArray.push(rndNum);
    }
}
...
function pg() {
    return src([siteconfig.build.links.source.pages + '/**/*.pug'])
        ...
        .pipe(injectdata(function() {
            return { rndNumArray: rndNumArray };
        }))
...
function askAllImages() {
    src(siteconfig.build.links.build.assets.img + '/**/*.{' + siteconfig.imagerename.formats.join(',') + '}')
        .pipe(through.obj(function(file, enc, cb) {
            ...
            if (siteconfig.imagerename.nonerename.indexOf(String(imgadr).trim()) === -1) {
                if ( thisimageList.indexOf( String(imgadr) ) === -1 ) {
                    thisimageList.push(String(imgadr));
    ...
    return src(siteconfig.build.links.build.assets.img + '/**/*.{' + siteconfig.imagerename.formats.join(',') + '}')
        .pipe(rename(function(path) {
            var cache = '',
                fileCache = '',
                realPathString = ( path.dirname !== '.' ) ? path.dirname + '/' + path.basename + path.extname : path.basename + path.extname;

            realPathString = win ? realPathString.replace("\\", "/") : realPathString;
            ...
            if (siteconfig.imagerename.nonerename.indexOf(String(realPathString).trim()) === -1) {
        ...
        .on('end', function() {
            ...
            for (var i = 0; i < thisimageList.length; i++) {
                var prestringimages = siteconfig.build.links.build.assets.img.split( siteconfig.build.links.build.core + '/' ).filter(item => { return item !== '' }).join(''),
                    imagereal = prestringimages + '/' + thisimageList[i],
                    imagerealcss = prestringimages.split('/').map((item, i) => { return (i == 0) ? '..' : item }).join('/') + '/' + thisimageList[i]
                    imagerename = prestringimages + '/' + thisnewImageList[i],
                    imagerenamecss = prestringimages.split('/').map((item, i) => { return (i == 0) ? '..' : item }).join('/') + '/' + thisnewImageList[i];
                console.log('\x1b[32m%s\x1b[0m', 'Переименование изображения: ' + imagereal + ' = ' + imagerename);
                console.log('\x1b[32m%s\x1b[0m', 'Переименование изображения для css: ' + imagerealcss + ' = ' + imagerenamecss);
                saveMassive.push([imagereal, imagerename]);
                saveMassiveCss.push([imagerealcss, imagerenamecss]);
            }
        });
};
...
function replaceAllImagesHTML() {
    return src(siteconfig.build.links.build.core + '/**/*.html')
        ...
        .pipe(replaceArray(saveMassiveCss))
        .pipe(dest(siteconfig.build.links.build.core));
};
...
exports.getRndNumArray = getRndNumArray;
...
exports['build:release'] = series(cleanbuild, jsonldeditor, jsoncontent, buildcopy, buildres, buildsender, buildrssphp, getRndNumArray, pg, scripts, icons, stylesbuild, askAllImages, replaceAllImagesHTML, replaceAllImagesCSS, replaceAllImagesJS, createsitemap, createrobots);
exports['build:dev'] = series(cleanbuild, jsoncontent, buildrssphp, parallel(buildcopy, buildres, scripts), icons, buildsender, getRndNumArray, pg, stylesbuild);
exports['build:none'] = series(cleanbuild, jsonldeditor, jsoncontent, buildcopy, buildres, buildsender, buildrssphp, getRndNumArray, pg, scriptscache, scripts, scriptscacheclean, icons, stylesbuild, askAllImages, replaceAllImagesHTML, replaceAllImagesCSS, replaceAllImagesJS, createsitemap, createrobots);
exports.default = series(series(cleanbuild, jsoncontent, buildrssphp, parallel(buildcopy, buildres, scripts), icons, buildsender, getRndNumArray, pg, stylesbuild), serve, browsersync);
